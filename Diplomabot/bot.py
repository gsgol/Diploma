...
connect = psycopg2.connect(user="temp",
                                  password="1234",
                                  host="127.0.0.1",
                                  port="5433",
                                  dbname="postgres_db")
...
connection = False
client = OpenAI(api_key= Diplomabot.config.API_KEY, base_url="https://api.proxyapi.ru/openai/v1")
def openAI(prompt):
                ...
                if not result['message']['from']['is_bot']:
                    ...
                    if("document" not in result["message"]):
                        ...
                        if '/login' in result['message']['text']:
                            ...
                            try:
                                connect = psycopg2.connect(user=f"{user}",
                                        password=f"{password}",
                                        host="127.0.0.1",
                                        port="5433",
                                        dbname=f"{dbname}")
